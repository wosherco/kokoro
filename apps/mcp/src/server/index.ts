...
server.tool(
  ...
  "Memories are Kokoro's way of storing information from different sources in a single place.",
  {
    ...
    integrationAccountIds: z
      ...
      .nullable()
      .describe(
        "If provided, only memories from these integration accounts will be returned",
      ),
    calendarIds: z
      .array(z.string().uuid())
      .optional()
      .nullable()
      .describe(
        "If provided, only memories from these calendars will be returned",
      ),
    tasklistIds: z
      .array(z.string().uuid())
      .optional()
      .nullable()
      .describe(
        "If provided, only memories from these tasklists will be returned",
      ),
    sortBy: z
      .enum(MEMORY_SORT_BY)
      .optional()
